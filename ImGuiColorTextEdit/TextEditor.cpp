...
#define _inject(...)				__VA_ARGS__

#if _inject(1)
//#define equals(...)   0
bool eqx(std::string::const_iterator b1, std::string::const_iterator& e1, std::vector<TextEditor::Glyph>::const_iterator b2, std::vector<TextEditor::Glyph>::const_iterator& e2) {
	if(b1 != e1 && b2 != e2) {
		const char* bp1=b1._Ptr;
		const char* ep1=e1._Ptr;
		const TextEditor::Glyph* bp2=b2._Ptr;
		const TextEditor::Glyph* ep2=e2._Ptr;
		for(;bp1 != ep1 && bp2 != ep2;bp1++,bp2++) {
			if(*bp1!=bp2->mChar) return false;
		}
	}
	return true;
}
#define equals(b1,e1,b2,e2,x)   eqx(b1,e1,b2,e2)
#endif
...
void TextEditor::ColorizeInternal()
{
	...
	_inject(auto iterationCounter = 0;)
